...
internal open class CborDecoder(
    ...
    override fun decodeChar(): Char = (decodeTag() but decodeString().singleOrNull())
    ...
    override fun decodeString(): String {
        ...
        return reader.read(size).toByteArray().decodeToString()
    }
    ...
    override fun decodeEnum(enumDescriptor: SerialDescriptor): Int =
        decodeTag() but enumDescriptor.getElementIndex(decodeString())
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeDecoder {
        if (reader.peek() hasMajor MAJOR_BYTE) return CborByteStringDecoder(reader, serializersModule).beginStructure(
            descriptor
        )
        if (descriptor == Descriptors.infiniteText) return CborInfiniteTextDecoder(
            reader,
            serializersModule
    ...
    override fun decodeValue(): Any {
        ...
        return when (it) {
            ...
            HEADER_NEGATIVE_64 -> ((reader.nextULong()).toLong().takeIf { it < 0 }?.xor(1L)
                ?: throw CborDecoderException.Default)
    ...
    private fun decodeMap() = decodeStructure(Descriptors.map) {
        ...
        while (true) {
            ...
            decodeElementIndex(Descriptors.any).takeIf { it != CompositeDecoder.DECODE_DONE }
                ?: throw CborDecoderException.Default
