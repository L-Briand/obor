...
internal class CborByteWriter(private val delegate: ByteVector) : CborWriter {
    private val buffer = UByteArray(9) { 0u }
    override fun write(byte: UByte) = delegate.add(byte.toByte())
    override fun write(bytes: UByteArray) = delegate.add(bytes.toByteArray(), 0, bytes.size)
    ...
    override fun writeHeader8(header: UByte, value: UByte) {
        buffer[0] = header
        buffer[1] = value
        delegate.add(buffer.toByteArray(), 0, 2)
    }
    ...
    override fun writeHeader16(header: UByte, value: UShort) {
        buffer[0] = header
        value.into(buffer, 1)
        delegate.add(buffer.toByteArray(), 0, 3)
    }
    ...
    override fun writeHeader32(header: UByte, value: UInt) {
        buffer[0] = header
        value.into(buffer, 1)
        delegate.add(buffer.toByteArray(), 0, 5)
    }
    ...
    override fun writeHeader64(header: UByte, value: ULong) {
        buffer[0] = header
        value.into(buffer, 1)
        delegate.add(buffer.toByteArray(), 0, 9)
    }
